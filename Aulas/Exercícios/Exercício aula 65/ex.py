...
Requisitos:
    A ordem do número duplicado é considerada a partir da segunda ocorrência (o número duplicado em si)
    Exemplo:
        [1, 2, 3, 3, 2, 1] -> 1, 2 e 3 são duplicados
...
def duplicados(array):
    ...
    if sorted(array) == sorted(list(set(array))):
        results.append([-1])
    else:
        for i in range(11):
            if i in array:
...
for list_of_integers in lista_de_listas_de_inteiros:
    print(duplicados(list_of_integers))
